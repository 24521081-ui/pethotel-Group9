  ...
  <form action="{{ route('booking.store') }}" method="POST" id="bookingForm" class="booking-form-v2">
    ...
    <div class="booking-box-v2">
      ...
      <div class="booking-room-list-v2">
        ...
        @php($isSoldOut = $roomType['is_sold_out'] ?? false)
        <button type="button" class="booking-room-card-v2 {{ $isSoldOut ? 'room-sold-out' : '' }}" aria-pressed="false"
          aria-disabled="{{ $isSoldOut ? 'true' : 'false' }}" data-room-id="{{ $roomType['id'] }}"
          ...
          @if ($isSoldOut)
          <span class="sold-out-badge">Tạm hết phòng trống</span>
          @endif
          ...
          <span class="room-meta-list">
            ...
            <span><i class="fa-solid fa-weight-scale"></i> Đến {{ $roomType['maxWeight'] }}kg</span>
            <span><i class="fa-solid fa-door-open"></i> Còn {{ $roomType['availableRoomsCount'] ?? 0 }} phòng</span>
          </span>

          <span class="room-detail-link">
            {{ $isSoldOut ? 'Không thể chọn' : 'Xem chi tiết' }}
    ...
    <div class="booking-box-v2 booking-step-panel" id="bookingDatePanel" hidden>
      <h3><i class="fa-regular fa-calendar"></i> Ngày lưu trú</h3>
    ...
    <div class="booking-box-v2 booking-step-panel" id="bookingPetPanel" hidden>
      ...
      <div class="pet-select-list" id="petSelectList">
        ...
        <div class="pet-item {{ $petInRoom ? 'ineligible pet-in-room' : '' }}" data-pet-id="{{ $pet['id'] }}" data-pet-name="{{ $pet['name'] }}"
          data-pet-species="{{ $pet['species'] }}" data-pet-breed="{{ $pet['breed'] }}" data-pet-sex="{{ $pet['sex'] }}"
          data-pet-weight="{{ $pet['weight'] }}" data-pet-in-room="{{ $petInRoom ? '1' : '0' }}"
          ...
          <label class="pet-item-select">
            ...
            <span class="pet-item-info">
              ...
              <span>{{ $pet['species'] }} · {{ $pet['breed'] }} · {{ $pet['sex'] }} · {{ $pet['weight'] }}kg</span>
            </span>
